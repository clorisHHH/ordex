...
import listOrderedIcon from './assets/file-icons/list_ordered_line.svg?raw';
...
import {DEFAULT_SCHEME,PRESETS,LEVEL_STYLES,SEPARATORS,normalizeScheme,formatNumber,formatOrdinal,sampleNumbers,findPreset,styleSample} from './numbering.js';
import {makeNode,locate,move,moveOut,height,containsFolder,swapSections,recycle,restoreEntry,discardTrashEntry,moveOutMany} from './model.js';
...
let trash=[],multi=false,checked=new Set(),removeImportNumbering=true,schemePanelOpen=false;
let numberingScheme=normalizeScheme(DEFAULT_SCHEME);
let folderRoots=[];
...
const savedView=currentCase?.view;if(savedView){active=savedView.active||active;selected=savedView.selected||active;collapsed=new Set(savedView.collapsed||[]);multi=!!savedView.multi;checked=new Set(savedView.checked||[]);removeImportNumbering=savedView.removeImportNumbering!==false;numberingScheme=normalizeScheme(savedView.numberingScheme);}
...
function applyCurrentNumberingPreference(){for(const snapshot of currentSnapshots()){applyNumberingPreference(snapshot.nodes,removeImportNumbering,numberingScheme);for(const entry of snapshot.trash||[])applyNumberingPreference([entry.node],removeImportNumbering,numberingScheme);}}
...
const separatorKey=sep=>sep==='.'?'schemeSepDot':sep==='-'?'schemeSepDash':sep==='_'?'schemeSepUnderscore':'schemeSepNone';
function schemePanelMarkup(){
 const presetId=findPreset(numberingScheme);
 const presets=PRESETS.map(preset=>`<button type="button" class="scheme-preset${presetId===preset.id?' is-active':''}" data-preset="${preset.id}" aria-pressed="${presetId===preset.id}"><b>${t('scheme_'+preset.id)}</b><em>${esc(sampleNumbers(preset).join(' / '))}</em></button>`).join('');
 const levels=numberingScheme.levels.map((style,index)=>`<label class="scheme-level"><span>${index+1}</span><select data-level="${index}" aria-label="${t('schemeLevels')} ${index+1}">${LEVEL_STYLES.map(item=>`<option value="${esc(item)}"${item===style?' selected':''}>${esc(styleSample(item))}</option>`).join('')}</select></label>`).join('');
 const separators=SEPARATORS.map(sep=>`<option value="${esc(sep)}"${sep===numberingScheme.separator?' selected':''}>${t(separatorKey(sep))}</option>`).join('');
 const sample=sampleNumbers(numberingScheme);
 return `<span class="scheme-control" data-scheme-control><button class="scheme-trigger${schemePanelOpen?' is-open':''}" type="button" data-scheme-trigger aria-expanded="${schemePanelOpen}" title="${t('schemeTitle')}"><span class="scheme-icon">${listOrderedIcon}</span><span class="scheme-trigger-sample">${esc(sample[sample.length-1])}</span><svg class="scheme-chevron" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m6 8 4 4 4-4"/></svg></button><div class="scheme-panel" data-scheme-panel${schemePanelOpen?'':' hidden'} role="group" aria-label="${t('schemeLabel')}"><div class="scheme-block"><span class="scheme-block-label">${t('schemePresets')}</span><div class="scheme-presets">${presets}</div></div><div class="scheme-block"><span class="scheme-block-label">${t('schemeLevels')}</span><div class="scheme-levels">${levels}</div></div><div class="scheme-block"><span class="scheme-block-label">${t('schemeSeparator')}</span><select class="scheme-separator" data-scheme-separator aria-label="${t('schemeSeparator')}">${separators}</select></div><div class="scheme-sample"><span>${t('schemeSample')}</span>${sample.map(item=>`<b>${esc(item)}</b>`).join('')}</div><p class="scheme-hint">${t('schemeHint')}</p></div></span>`;
}
let closeSchemePanel;
function mountSchemeControl(anchor){
 const slot=document.createElement('span');slot.className='scheme-slot';slot.innerHTML=schemePanelMarkup();anchor.after(slot);
 const control=slot.querySelector('[data-scheme-control]'),trigger=control.querySelector('[data-scheme-trigger]'),panel=control.querySelector('[data-scheme-panel]');
 const close=()=>{if(!schemePanelOpen)return;schemePanelOpen=false;trigger.classList.remove('is-open');trigger.setAttribute('aria-expanded','false');panel.hidden=true;document.removeEventListener('pointerdown',dismiss);document.removeEventListener('keydown',onKey);if(closeSchemePanel===close)closeSchemePanel=null;};
 const dismiss=event=>{if(!control.contains(event.target))close();};
 const onKey=event=>{if(event.key==='Escape'){event.preventDefault();close();trigger.focus();}};
 trigger.onclick=()=>{if(schemePanelOpen){close();return;}schemePanelOpen=true;trigger.classList.add('is-open');trigger.setAttribute('aria-expanded','true');panel.hidden=false;closeSchemePanel=close;document.addEventListener('pointerdown',dismiss);document.addEventListener('keydown',onKey);};
 control.querySelectorAll('[data-preset]').forEach(button=>{button.onclick=()=>{const preset=PRESETS.find(item=>item.id===button.dataset.preset);if(!preset)return;numberingScheme=normalizeScheme(preset);applySchemeChange();};});
 control.querySelectorAll('[data-level]').forEach(select=>{select.onchange=()=>{const levels=[...numberingScheme.levels];levels[Number(select.dataset.level)]=select.value;numberingScheme=normalizeScheme({levels,separator:numberingScheme.separator});applySchemeChange();};});
 control.querySelector('[data-scheme-separator]').onchange=event=>{numberingScheme=normalizeScheme({levels:numberingScheme.levels,separator:event.target.value});applySchemeChange();};
}
function applySchemeChange(){applyCurrentNumberingPreference();save();render();toast(t('schemeUpdated'));}
...
function deleteItems(ids){
 ...
 commit(()=>{recycle(nodes,trash,ids,numberingScheme);checked.clear();});
 ...
 const refresh=()=>{
  ...
  content.querySelectorAll('.trash-restore').forEach(button=>button.onclick=()=>{
   ...
   try{
    ...
    else if(caseItem===currentCase){commit(()=>{number=restoreEntry(nodes,trash,button.dataset.entry,numberingScheme);});status.textContent=number?t('restoredPosition',{number}):'';}
    else{const workspace=caseItem.workspace,before=structuredClone({nodes:workspace.nodes,trash:workspace.trash});number=restoreEntry(workspace.nodes,workspace.trash,button.dataset.entry,numberingScheme);workspace.history=[...(workspace.history||[]),before].slice(-60);workspace.future=[];caseItem.updatedAt=Date.now();save();render();status.textContent=t('restoredTo',{project:caseItem.name,number});}
    refresh();
...
function dropPreview(d){
 ...
 try{if(copy.some(n=>n.id===dragged)&&copy.some(n=>n.id===d.target))swapSections(copy,dragged,d.target);else move(copy,dragged,d.target,d.position);return {number:formatNumber(locate(copy,dragged).path,numberingScheme)};}
...
function wireDrop(el,get){el.ondragover=e=>{e.preventDefault();e.stopPropagation();const d=get(e);clearDrop();el.classList.add('drop-'+(d.position||'inside'));
 ...
 if(caption){
  ...
  const targetNumber=formatNumber(locate(nodes,d.target).path,numberingScheme);
...
function currentView(){return {active,selected,collapsed:[...collapsed],multi,checked:[...checked],removeImportNumbering,numberingScheme,scrollY:window.scrollY};}

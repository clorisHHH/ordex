...
两个版本并存，导入、拖拽排序、回收站、导出与预览的操作方式完全一致，可按需要选择：

- **2.0.0（最新 · Latest）** — 可切换编号形式：除 `1.1.1` 这类数字编号外，还支持 `a b c`、`A B C`、`i ii iii`、`I II III`、`一 二 三` 及括号形式，可逐层设置；另支持多选批量移出上一层。默认仍是数字编号，不切换时与 1.0 完全相同。[下载 Mac / Windows](https://github.com/clorisHHH/ordex/releases/tag/v2.0.0)
- **1.0.0** — 首发版，只有纯数字编号。[下载 Mac / Windows](https://github.com/clorisHHH/ordex/releases/tag/v1.0.0)

Two versions are available and maintained side by side; importing, drag-and-drop, trash, export and preview work the same in both:

- **2.0.0 (Latest)** — adds switchable numbering schemes (`a b c`, `A B C`, `i ii iii`, `I II III`, Chinese numerals, with per-level settings) and batch move-out for multi-selection. It still defaults to decimal numbering, so it behaves like 1.0 until you switch. [Download for Mac / Windows](https://github.com/clorisHHH/ordex/releases/tag/v2.0.0)
- **1.0.0** — the initial release with decimal numbering only. [Download for Mac / Windows](https://github.com/clorisHHH/ordex/releases/tag/v1.0.0)
